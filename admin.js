...
async function loadSettings(){try{const r=await get('adminGetSettings');const keys=['SYSTEM_NAME','INSTITUTE_NAME','INSTITUTE_YEAR','INSTITUTE_DATE_FROM','INSTITUTE_DATE_TO','VENUE','VENUE_MAP_URL','REGISTRATION_OPEN','REGISTRATION_DEADLINE','MAX_PARTICIPANTS','CERTIFICATE_MIN_ATTENDANCE','CERTIFICATE_REQUIRE_QUIZ','CERTIFICATE_REQUIRE_EVALUATION','CONTACT_PHONE','CONTACT_EMAIL','WHATSAPP_NUMBER','LOGO_URL','NOTICE_MARQUEE','SYSTEM_STATUS','VERSION'];$('#settingsGrid').innerHTML=keys.map(k=>`<div class="field"><label>${esc(k.replaceAll('_',' '))}</label>${['REGISTRATION_OPEN','SYSTEM_STATUS','CERTIFICATE_REQUIRE_QUIZ','CERTIFICATE_REQUIRE_EVALUATION'].includes(k)?selectSetting(k,r.settings[k]||''): `<input data-setting="${k}" value="${esc(r.settings[k]||'')}" ${k==='VERSION'?'readonly':''}>`}</div>`).join('');if(!canWrite())document.querySelectorAll('[data-setting]').forEach(x=>x.disabled=true)}catch(e){toast(e.message,'error')}}
function selectSetting(k,v){const opts=k==='SYSTEM_STATUS'?['ACTIVE','MAINTENANCE']:['YES','NO'];return `<select data-setting="${k}" ${!canWrite()?'disabled':''}>${opts.map(o=>`<option ${o===v?'selected':''}>${o}</option>`).join('')}</select>`}
$('#saveSettingsBtn').onclick=async()=>{if(!canWrite()){toast('Viewer account cannot make changes.','error');return}const b=$('#saveSettingsBtn'),obj={};document.querySelectorAll('[data-setting]').forEach(x=>obj[x.dataset.setting]=x.value);loadBtn(b,true,'Saving');try{await post('adminSaveSettings',{settings:JSON.stringify(obj)});toast('Settings saved.','success')}catch(e){toast(e.message,'error')}finally{loadBtn(b,false)}};
