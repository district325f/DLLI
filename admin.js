...
async function loadSettings(){try{const r=await get('adminGetSettings');const keys=['SYSTEM_NAME','DISTRICT_NAME','INSTITUTE_NAME','INSTITUTE_YEAR','INSTITUTE_DATE_FROM','INSTITUTE_DATE_TO','VENUE','VENUE_MAP_URL','REGISTRATION_OPEN','REGISTRATION_DEADLINE','MAX_PARTICIPANTS','CONTACT_PHONE','CONTACT_EMAIL','WHATSAPP_NUMBER','LOGO_URL','NOTICE_MARQUEE','MARQUEE_SPEED','MARQUEE_BOLD','MARQUEE_TEXT_COLOR','MARQUEE_BG_COLOR','MARQUEE_FONT_SIZE','MARQUEE_DIRECTION','ATTENDANCE_URL','ATTENDANCE_BUTTON_TEXT','ATTENDANCE_OPEN_NEW_TAB','SYSTEM_STATUS','VERSION'];$('#settingsGrid').innerHTML=keys.map(k=>`<div class="field"><label>${esc(k.replaceAll('_',' '))}</label>${['REGISTRATION_OPEN','SYSTEM_STATUS','MARQUEE_BOLD','MARQUEE_DIRECTION','ATTENDANCE_OPEN_NEW_TAB'].includes(k)?selectSetting(k,r.settings[k]||''): `<input data-setting="${k}" value="${esc(r.settings[k]||'')}" ${k==='VERSION'?'readonly':''}>`}</div>`).join('');configureAttendanceAdmin(r.settings||{});if(!canWrite())document.querySelectorAll('[data-setting]').forEach(x=>x.disabled=true)}catch(e){toast(e.message,'error')}}
